...
select distinct m.movie_id, m.title, m.movie_year, m.rating, m.votes
from (select * from movies m where m.movie_id not in (select w.movie_id from watched w where w.customer_id = %s)) as m,
(select avg(m.votes) as av from movies m, watched w where w.customer_id = %s and w.movie_id = m.movie_id) as n
where m.votes > n.av
order by m.votes desc
limit 10

select distinct m.movie_id, m.title, m.movie_year, m.rating, m.votes 
from (select g.genre_name, max(m.votes) as v from (select * from movies m where m.movie_id not in (select w.movie_id from watched w where w.customer_id = %s)) as m, genres g where g.movie_id = m.movie_id group by g.genre_name) as n1,
movies m, genres g2, (select g.genre_name from watched w, genres g where w.customer_id = %s and g.movie_id = w.movie_id) as n2
where m.votes = n1.v and g2.movie_id = m.movie_id and g2.genre_name = n1.genre_name and n2.genre_name = n1.genre_name
